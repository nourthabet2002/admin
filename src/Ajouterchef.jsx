...
function AddChef() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if chef with the selected category already exists
      const existingChefResponse = await axios.get(`http://localhost:7000/chefs?categorieId=${formData.categorieId}`);
      if (existingChefResponse.data.length > 0) {
        setError("Un chef existe déjà pour cette catégorie.");
        return;
      }
      ...
      console.error('Error adding chef:', error);
      if (error.response && error.response.status === 400) {
        setError("Chef existe déjà");
      } else {
        setError('An error occurred while adding the chef.');
      }
    }
  };
